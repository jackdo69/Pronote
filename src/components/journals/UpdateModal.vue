    ...
    methods: {
      remove () {
        journalRepository.remove(this.journal, (err) => {
          if (err) {
            EventBus.$emit('alert', {type: 'error', message: 'Failed to remove journal'})
            return
          }
          this.dismissModal()
        })
...
<style>
  ...
  form.edit-journal input, form.edit-journal textarea{
    width: 100%;
    max-width: 100%;
    border: none;
    padding: 4px;
  ...
  form.edit-journal button:hover, form.edit-journal button:focus{
    opacity: 1;
  }
