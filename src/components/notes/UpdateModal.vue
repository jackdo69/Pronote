<template>
  <transition name="modal">
    <div v-if="note" class="backdrop" @click.self="dismissModal">
      <form class="edit-note" v-on:submit.prevent="update" >
    ...
    methods: {
      remove () {
        noteRepository.remove(this.note, (err) => {
          if (err) {
            EventBus.$emit('alert', {type: 'error', message: 'Failed to remove note'})
            return
          }
          this.dismissModal()
        })
      ...
      update () {
        noteRepository.update(this.note, (err) => {
          if (err) {
            EventBus.$emit('alert', {type: 'error', message: 'Failed to update note'})
            return
          }
          this.dismissModal()
        })
